...
private:
    ...
    float constrainValue(float value, float min_val, float max_val) {
    ...
    bool getInterpolatedOffsets(float voltage, PWMCoefficients &coeffs,
        ...
        if (voltage < 3.6 || voltage > 4.2) {
            voltage = constrainValue(voltage, MIN_VOLTAGE, MAX_VOLTAGE);
...
public:
    ...
    bool updateVoltage(float voltage) {
        if (voltage < MIN_VOLTAGE || voltage > MAX_VOLTAGE) {
            last_error = EKFError::VOLTAGE_OUT_OF_BOUNDS;
            current_voltage = constrainValue(voltage, MIN_VOLTAGE, MAX_VOLTAGE);
        } else {
